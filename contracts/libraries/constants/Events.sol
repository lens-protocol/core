...
library Events {
     ...
     *
     * @param wallet The address whose Profile Guardian state change is being triggered.
     * @param enabled True if the Profile Guardian is being enabled, false if it is being disabled.
     * @param profileGuardianDisablingTimestamp The UNIX timestamp when disabling the Profile Guardian will take effect,
     * if disabling it. Zero if the protection is being enabled.
     * @param timestamp The UNIX timestamp of the change being triggered.
     */
    event ProfileGuardianStateChanged(
        address indexed wallet,
        bool indexed enabled,
        uint256 profileGuardianDisablingTimestamp,
        uint256 timestamp
    );
}
