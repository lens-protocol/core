...
The environment is built using Docker Compose, note that your `.env` file must have an `ALCHEMY_KEY` `INFURA_KEY`, or `QUICKNODE_KEY` variable, and an optional `MNEMONIC` and `ETHERSCAN_KEY`, defined like so:
